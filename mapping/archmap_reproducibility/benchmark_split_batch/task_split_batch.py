...
def main(query_name):
    ...
    if model_type!= "scpoli":
        ...
        if "X_latent_qzm" in reference_adata.obsm:
...
# Break queries into separate datasets and map separately
def separate_datasets(query, batch, atlas):
    queries=[]
    # for dataset in query.obs[batch].unique()[1]:
    dataset="Duong_lungMAP_unpubl"

    query_sub = query[query.obs[batch]==dataset]
    query_sub.write(path + f"queries/{atlas}_{dataset}_{batch}.h5ad")

    queries.append(f"{atlas}_{dataset}_{batch}")
...
if __name__  == "__main__":
    ...
    # queries = ["hlca_healthy_split", "hlca_healthy_full"]
    ...
    query = sc.read(path + "hlca_healthy_ext.h5ad")

    queries = separate_datasets(query, batch, "hlca")
